...
class CNN_Model():
    def __get_model(self):
        ...
        hidden = Dropout(0.2) (hidden)
        hidden = Conv2D(8,(3,3),strides = (1,1),padding = 'same') (hidden)
        hidden = Dropout(0.2) (hidden)
        hidden = Conv2D(4,(3,3),strides = (1,1),padding = 'same') (hidden)
        hidden = Dropout(0.2) (hidden)
        out = Conv2D(1,(3,3),strides = (1,1),padding = 'same') (hidden)
        model = Model(x,out)
...
class CNN_Data_Formatter():
    ...
    def sampling(self, dataset):
        ...
        r = np.random.randint(len(dataset)-number_of_input_days)
        input_start = r
        input_end = input_start + number_of_input_days

        x = dataset[input_start:input_end]
        y = dataset[input_end:input_end+1]

        return np.reshape(x,self.input_shape), np.reshape(y,self.output_shape)

    def get_minibatch(self, x_data, batch_size):
        minibatch_x = []
        minibatch_y = []
        for _ in range(batch_size):
            x, y = self.sampling(x_data)
            minibatch_x.append(x)
            minibatch_y.append(y)
        return np.reshape(np.asarray(minibatch_x),(batch_size,self.input_shape[0],self.input_shape[1],self.input_shape[2])), np.reshape(np.asarray(minibatch_y),(batch_size, self.output_shape[0],self.output_shape[1]))